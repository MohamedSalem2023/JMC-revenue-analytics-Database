...
export function parseExcelDate(dateValue: any): Date | null {
  ...
  if (typeof dateValue === 'string') {
    ...
    if (!isNaN(parsed.getTime())) {
       ...
       // Handle ISO strings (e.g., from localforage storage fallback or Firestore/MongoDB)
       if (trimmed.includes('T') && trimmed.endsWith('Z')) {
         // If the time portion is T20–T23:00:00Z, it's a local midnight date that was stored in UTC.
         // This covers UTC+1 (T23), UTC+2 (T22), UTC+3 (T21, Saudi Arabia/Kuwait/Qatar),
         // and UTC+4 (T20, UAE/Oman). We restore the ORIGINAL local date using local components.
         if (/T(2[0-3]):00:00/.test(trimmed)) {
...
export function processRawData(data: RawDataRow[]): ProcessedRow[] {
  ...
  const sortedData = (dataWithParsedDates as any[]).sort((a, b) => {
    return a.parsedDate.getTime() - b.parsedDate.getTime();
  });

  return sortedData.map((row) => {
    const date = row.parsedDate as Date;
