...
export const Filters: React.FC<FiltersProps> = ({
  ...
  return (
    <div className="w-80 bg-white border-r border-slate-100 h-screen sticky top-0 p-6 overflow-y-auto hidden lg:block">
      <div className="flex flex-col items-center gap-3 mb-8 pb-6 border-b border-slate-100">
        <img 
        ...
        />
        <h2 className="font-bold text-slate-900 text-center text-sm">مركز طوارئيات للعناية الطبية - الجبيل</h2>
      </div>

      <div className="flex items-center gap-2 mb-6">
        <Filter className="w-5 h-5 text-blue-600" />
        <h2 className="font-bold text-slate-900">Navigation & Filters</h2>
      </div>

      <div className="mb-8 space-y-2">
        <button 
          onClick={() => setActiveTab("revenue")} 
          className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all", activeTab === "revenue" ? "bg-blue-600 text-white shadow-md" : "text-slate-600 hover:bg-slate-50")}
        >
          <TrendingUp className="w-5 h-5" /> Revenue
        </button>
        <button 
          onClick={() => setActiveTab("volume")} 
          className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all", activeTab === "volume" ? "bg-blue-600 text-white shadow-md" : "text-slate-600 hover:bg-slate-50")}
        >
          <Users className="w-5 h-5" /> Volume
        </button>
        <button 
          onClick={() => setActiveTab("cpp")} 
          className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all", activeTab === "cpp" ? "bg-blue-600 text-white shadow-md" : "text-slate-600 hover:bg-slate-50")}
        >
          <PieChartIcon className="w-5 h-5" /> CPP Analysis
        </button>
        {isAdmin && (
          <button 
            onClick={() => setActiveTab("data")} 
            className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all", activeTab === "data" ? "bg-blue-600 text-white shadow-md" : "text-slate-600 hover:bg-slate-50")}
          >
      ...
      <div className="space-y-8">
        ...
        <section>
          ...
          <select
            ...
            onChange={(e) => onFilterChange({ ...selectedFilters, year: e.target.value })}
